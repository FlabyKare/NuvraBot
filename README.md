...
- автоматические категории: ссылки, посмотреть, разработка, купить, почитать, файлы;
- SQLite + FTS5 для быстрого полнотекстового поиска;
- embeddings и семантический поиск человеческим языком;
- избранное, прочитано/не прочитано, напоминания на завтра или через месяц;
...
Без `OPENAI_API_KEY` используется локальный FTS5. Если ключ задан, при сохранении создаётся embedding, а поиск смешивает семантическую близость с полнотекстовым совпадением.
...
После сохранения доступны inline-действия: избранное, прочитано, напомнить завтра, напомнить через месяц и суммаризировать.
...
- ручные теги и пользовательские коллекции;
